...
init([Ip, Port, ServeId, Gateways]) ->
    ...
    error_logger:info_msg("~s The mod_disperse is OK! . ~n", [misc:time_format(now())]),
    {ok, State}.
...
get_server_list(AccId,[Server|LeftServerList],PreDomain,ServList,PlayerList) ->
	...
	case NewPlayerList of
		...
		_ ->
			ServPlayerList = 
				[{SP#server_player.sex,SP#server_player.nick} 
				 || SP <- NewPlayerList , SP#server_player.accid =:= AccId , SP#server_player.serv_id =:= Server#server.id] ,
			case ServPlayerList of
				[] ->
					[Sex,Nick] = [0,[]] ;
				_ ->
					[{Sex,Nick}|_] = ServPlayerList
			end 
	end ,
	NewServList = [{Server#server.id,Server#server.domain,Name,Server#server.ip,Server#server.port,Server#server.state,Sex,Nick}|ServList] ,
