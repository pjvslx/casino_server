...
%%Port:端口
init() ->
    io:format("==============game_server_reader init==============~n"),
...
login_parse_packet(Socket, Client) ->
    ...
    receive
        ...
        {inet_async, Socket, Ref, {ok, <<Len:16, Cmd:16>>}} ->
            io:format("+++++++login_parse_packet Cmd = [~p]~n",[Cmd]),
                                ...
                                {ok, enter, [Id]} ->
										case lib_account:ban_account(Id) of
											...
											unban ->
		                                      case Client#client.login == 1 of
		                                          true ->
		                                              case mod_login:login(start, [Id, Client#client.account_id], Socket) of
		                                                  {ok, Pid} ->
...
do_parse_packet(Socket, Client) ->
    ...
    receive
                ...
                {ok, BinData} ->  
                    case routing(Client, Cmd, BinData) of
                        ...
                        {ok, Data} ->  
                            case catch gen:call(Client#client.player_pid, '$gen_call', {'SOCKET_EVENT', Cmd, Data}, 10*1000) of
                                {ok, _Res} ->
                                    do_parse_packet(Socket, Client);
                                {'EXIT',Reason} -> 
									io:format("Lost player connection cmd : ~p reason ~p ~n",[Cmd,Reason]),
                        ...
                        Other2 ->
							...
							io:format("[SYSTEM] player do lost in cmd : ~p data: ~p ~n",[Cmd,Other2]),
        ...
        {inet_async, Socket, Ref, {error,timeout}} ->
            case Client#client.timeout >= ?HEART_TIMEOUT_TIME  of
                true  ->
					io:format("timeout Lost player connection ~n",[]),
        ...
        %%用户断开连接或出错
        Other ->
		   io:format("Lost player connection in other reason ~p ~n",[Other]),
           do_lost(Client, 0, Other, 5)
    end.

%%登录断开连接
login_lost(Socket, _Client, _Location, Reason) ->
    io:format("login_lost Reason = [~p] ~n",[Reason]),
    KillReason = 
		case Reason of
...
do_lost(Client, _Cmd, Reason, _Location) ->
	io:format("[DO_LOST]Protocol Number ~p~n",[_Cmd]),  
	KillReason = 
		case Reason of
			{inet_async,_,_,{error,closed}} ->
				shutdown ;
