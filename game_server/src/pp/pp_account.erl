                ...
                true ->
                    ...
                    case lib_account:create_role(Accid, Accname, Sex, Name) of    
                         RoleId when is_integer(RoleId) ->
							...
							io:format("33333 10003 s->c code = ~p  createRoleId = ~p",[1,RoleId]),
                            lib_send:send_one(Socket, BinData);
                        _Other ->
                            %%角色创建失败
                            {ok, BinData} = pt_10:write(10003, [0, 0]),
							io:format("44444 10003 s->c code = ~p  createRoleId = ~p",[0,0]),
                            lib_send:send_one(Socket, BinData)
                    end
            end
    end;
